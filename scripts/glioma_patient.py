...
# run twice for now
fig_save_path = './figures/glioma/fitting/pearson_patients/patient4_h_inds/'
random_freqs = False
freq_normal = False
random_init = False
run = True
...
if run:
    ...
    for i, p in enumerate(patient_ids):  
        ...
        control_pars = h_tumor
        #bounds = [ (0.1, 20) for _ in range(len(control_pars)) ]  # for a and b
        #bounds = [ (-50, 50) for _ in range(len(control_pars)) ]  # for decays
        bounds = [ (0.0, 50) for _ in range(len(control_pars)) ]  # for h
        if thres_h == -1:
            bounds.append((0,1.0))
        ...
        def optimize(varl): 
            ...
            minimize = differential_evolution(error_FC, bounds, args=(DE, mean_W, tspan, step, \
                            ...
                            False, False, zero_scale, y0, tumor_inds, objective, freq_normal), \
...
for p in range(n_patients):
    ...
    for npar in range(n_pars):
        ...
        hmin = np.nanmin(healthy_par)-1
        hmax= np.nanmax(healthy_par)+1
        pmin = np.nanmin(patient_par)-1
        pmax= np.nanmax(patient_par)+1
        ax.set_xlim([np.amin([hmin, pmin]),np.amax([hmax, pmax])])
        ...
        plt.xlim([np.amin([hmin, pmin]),np.amax([hmax, pmax])])
